...
# start
# The superior directory of bin and lib
FAKEROOT_PREFIX=/data/local/tmp/usr
# bin directory
FAKEROOT_BINDIR=/data/local/tmp/usr/bin

USEABSLIBPATH=0
LIB=libfakeroot.so
# Set your PATH and directory
PATHS=/data/local/tmp/usr/lib:${FAKEROOT_PREFIX}/lib64/libfakeroot:${FAKEROOT_PREFIX}/lib32/libfakeroot
...
if [ "$WAITINTRAP" -eq 0 ]; then
  ...
  trap 'FAKEROOTKEY=$FAKEROOTKEY LD_LIBRARY_PATH="$PATHS" LD_PRELOAD="$LIB" ls -l / >/dev/null 2>&1; while kill -s HUP $PID 2>/dev/null; do sleep 0.1; done' EXIT INT
  #trap 'FAKEROOTKEY=$FAKEROOTKEY LD_PRELOAD="$LIB" ls -l / >/dev/null 2>&1; while kill -s HUP $PID 2>/dev/null; do sleep 0.1; done' EXIT INT
fi
...
if test -z "$*"; then
  FAKEROOTKEY=$FAKEROOTKEY LD_LIBRARY_PATH="$PATHS"  LD_PRELOAD="$LIB" ${SHELL:-/data/data/com.termux/files/usr/bin/sh}
  #FAKEROOTKEY=$FAKEROOTKEY  LD_PRELOAD="$LIB" ${SHELL:-/data/data/com.termux/files/usr/bin/sh}
  RESULT=$?
else
